...
class API:
    ...
    def __init__(self, city: str, ras_key: str, latitude: float, longitude: float):
    ...
    def set_flag(self, color: int):
        try:
            requests.post(
                self.url + "/set_flag",
                json={"key": self.RASPBERRY_KEY, "color": color, "city": self.city},
            )
            print("change flag :)")
    ...
    def delete_alert_by_city(self):
        requests.post(
            self.url + "/delete_alert_by_city",
            json={"key": self.RASPBERRY_KEY, "city": self.city},
        )
    ...
    def add_picture_alert_or_moment(self, path_file: str):
        requests.post(
            self.url + "/add_picture_alert_or_moment",
            json={
                "key": self.RASPBERRY_KEY,
            },
    ...
    def get_picture(self):
        picture_base_64 = requests.post(
            ...
            json={"city": self.city + "-not-trained" ".png"},
        )
    ...
    def get_cache_size(self, detector_id: int):
        response = requests.get(
            f"{self.url}/cache_size", params={"detector_id": detector_id}
        )
        response.raise_for_status()
